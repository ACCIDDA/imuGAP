...
#' process and validate these inputs:
#' - `canonicalize_locations()` validates the geographic hierarchy structure (checks for unique root, no cycles, no duplicates) and assigns layers.
#' - `canonicalize_observations()` validates observation counts (e.g. positive cases <= sample_n) and prepares censoring indicators.
#' - `canonicalize_populations()` validates cohort, age, dose, and weight metadata and maps them to canonical observation and location IDs.
#'
#' Each function returns a `data.table` with canonicalized sequential IDs (such as
#' `loc_c_id` or `obs_c_id`) matching the indexing expected by the sampler.
#'
