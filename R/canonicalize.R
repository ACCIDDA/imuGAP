...
#' The `[sampling()]` sampler supports hierarchical models of arbitrary depth
#' (e.g., 1-layer root only, 2-layer root + sub-locations, 3-layer like
#' state -> county -> school, or deeper regional partitions). This method checks
#' location structure validity and returns a canonical version including layer
#' membership.
#'
...
#' - **branching constraints**: every location node must have either **0 offspring**
#'   (leaf location) or **at least 2 offspring** (>= 2 children). Single-child
#'   chains (a parent location with exactly 1 child) are rejected to ensure
#'   variance components are statistically identifiable across adjacent layers.
#'
