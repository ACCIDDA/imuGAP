...
`imuGAP` provides four bundled datasets (`locations_sim`, `observations_sim`, `populations_sim`, and `target_sim`) alongside latent parameter fixtures (`latent_params_sim`, `fit_sim`, `predict_sim`) that reflect an underlying vaccination process model and surveillance streams across a multi-resolution hierarchical population structure.
...
The `populations` table provides essential metadata for all observations—defining the discrete location, age, cohort, and dose associated with each survey outcome. Every record in `observations` receives one or more entries in `populations`:

- **Direct (1:1) observations**: A single row with `weight = 1.0` maps the observation directly to a discrete population slice (such as a single-cohort kindergarten survey at age 5).
- **Mixed observations**: When a survey mixes multiple underlying populations (such as cross-sectional surveys spanning multiple age groups or cohorts), `populations` captures this mixture using multiple rows sharing the same `obs_id`, each assigned a fractional weight (e.g. equal weights or demographic population shares summing to 1.0).
...
For single-cohort kindergarten surveys, `obs_id` has a 1:1 mapping with `weight = 1.0`. For multi-age surveys (such as TeenVaxView at `obs_id == 761`), multiple rows describe the mixture across ages:
...
state_obs <- copy(subset(observations_sim, loc_id == "State"))
...
single_cohort_obs <- subset(state_obs, is.na(age_max) | age_max == age_min + 1L)
multi_cohort_obs <- copy(subset(state_obs, !is.na(age_max) & age_max > age_min + 1L))
...
county_obs <- copy(subset(observations_sim, loc_id %in% county_names))
...
county_latent_obs <- subset(county_latent, cohort_min <= max_obs_cohort)
county_latent_unobs <- subset(county_latent, cohort_min >= max_obs_cohort)
...
sch_obs <- copy(subset(observations_sim, loc_id %in% sch_locs$loc_id))
...
school_latent_obs <- subset(school_latent, cohort_min <= max_obs_cohort)
school_latent_unobs <- subset(school_latent, cohort_min >= max_obs_cohort)
