...
Fitting Bayesian hierarchical models via Stan requires assessing whether MCMC chains have successfully converged, whether the sampler operated without pathologies (such as divergences or energy Bayesian Fraction of Missing Information warnings), and whether the model recovered the underlying epidemiological parameters.
...
We load the bundled `fit_sim` object, which wraps the underlying Stan model fit along with dataset and options metadata:
...
The state-level baseline propensity curve is parameterized as a B-spline over birth cohorts with coefficients $\beta_{\text{bs}}$. Below, we examine trace plots across all 4 MCMC chains alongside within-chain medians and the true simulation parameters (dashed red lines):
...
Location offsets represent deviation in vaccination propensity from the parent region. Below, we plot trace lines for Scruggs, Simone, and Watson counties.
...
The unconstrained rate parameters $\lambda_{\text{raw}}$ govern the speed at which eligible individuals receive vaccine doses once reaching age eligibility. Trace plots on the exponentiated rate scale demonstrate excellent chain mixing:
...
- For an overview of the core model workflow, see **[Getting Started with imuGAP](imuGAP.html)**.
