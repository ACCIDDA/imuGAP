...
invalid_obs[sample(nrow(observations_sim), 5), positive := sample_n + 10]

# This will fail validation and throw an error:
tryCatch(
  canonicalize_observations(invalid_obs),
  error = function(e) message("Caught expected error:\n\t", e$message)
)
```
...
# Calculate the posterior mean coverage probability for each observation
summary_predict <- summary(predict_sim)
summary_predict[age %in% 2:6 & loc_id == "State"]
```
...
summary_predict |>
  ...
  ggplot() +
  aes(x = age) +
  geom_line(aes(y = q50, color = loc_id, linetype = "median")) +
  geom_line(aes(y = q2_5, color = loc_id, linetype = "lower")) +
  geom_line(aes(y = q97_5, color = loc_id, linetype = "upper")) +
  theme_bw() +
  theme(
  ...
  scale_linetype_manual(
    NULL,
    values = c("median" = "solid", "lower" = "dashed", "upper" = "dotted")
  ) +
  labs(
...
focal_view <- summary_predict |> subset(
  loc_id %in% scruggs_schools & dose == 2 & age > 4
)
school_order <- focal_view[age == 5][order(q50), loc_id]
ggplot(focal_view) +
  geom_point(aes(x = age, y = q50, color = loc_id)) +
  theme_bw() +
  scale_color_discrete(breaks = rev(school_order)) +
  theme(
    legend.position = "inside", legend.position.inside = c(0.5, 0.5)
  ) +
  labs(
    x = "Age", y = "Coverage", color = "School",
...
latent_ref$coverage <- latent_params_sim$coverage[predict_sub$target$obs_c_id]
...
ggplot() +
  ...
  geom_boxplot(
    data = draws_df,
    mapping = aes(group = interaction(age, loc_id))
  ) +
  geom_point(
    data = latent_ref,
