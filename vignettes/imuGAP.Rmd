...
The populations dataset acts as observation metadata, mapping each observation ID (`obs_id`) to the corresponding location, birth cohort, age at observation, vaccine dose, and observation weight (`weight`). We validate and canonicalize it using `canonicalize_populations()`.

For observations that record coverage for a single cohort and age (such as kindergarten entry surveys), there is a 1:1 mapping where each `obs_id` has a single row with `weight = 1.0`:

```{r populations-single}
data("populations_sim", package = "imuGAP")
head(populations_sim)
```

However, composite observations (such as TeenVaxView surveys spanning multiple age groups) aggregate individuals across multiple cohorts and ages into a single observation. In such cases, `populations_sim` contains multiple rows for the same `obs_id`, with fractional weights summing to `1.0`:
...
1. **State Level (ChildVaxView, SchoolVaxView, TeenVaxView)**: Observations across cohorts spanning doses 1 and 2, plotted against the underlying lifetime uptake propensity $1 - \phi_{st}$.
...
### Summary Statistics of Observation Deviations

To evaluate how closely the randomly drawn observation samples match the expected underlying
coverage for each location and data stream, we can calculate summary statistics on the log-odds
(logit) scale.

For each observation $i$, the observed proportion is $\hat{p}_i = y_i / n_i$ (with
Haldane-Anscombe continuity correction $\tilde{p}_i = \frac{y_i + 0.5}{n_i + 1}$ for empirical
logits), and the expected latent coverage is $\mu_i = \mathbb{E}[Y_i] / n_i$. The log-odds
residual is defined as:

$$\Delta\text{logit}_i = \text{logit}(\tilde{p}_i) - \text{logit}(\mu_i)$$

Aggregating these residuals across all observations by location type and observation source
characterizes sampling variation and confirms that the simulated observations are well-calibrated
(mean log-odds residual $\approx 0$, odds ratio $\approx 1.0$):

```{r observation-summary-table}
obs <- copy(observations_sim)

# Identify location type and observation stream
obs[, loc_type := factor(fcase(
  loc_id == "State", "State",
  loc_id %in% c("Scruggs", "Simone", "Watson"), "County",
  default = "School"
), levels = c("State", "County", "School"))]

obs[, observation := factor(fcase(
  loc_type == "State" & dose == 1 & age_min == 2, "ChildVaxView (Dose 1, Age 2)",
  loc_type == "State" & dose == 1 & age_min == 3, "ChildVaxView (Dose 1, Age 3)",
  loc_type == "State" & dose == 2 & age_min == 5, "SchoolVaxView (Dose 2, Age 5)",
  loc_type == "State" & dose == 2 & age_min == 14, "TeenVaxView (Dose 2, Ages 14-18)",
  loc_type == "County" & dose == 2 & age_min == 11, "6th Grade Survey (Dose 2, Age 11)",
  loc_type == "School" & dose == 2 & age_min == 5, "Kindergarten Entry (Dose 2, Age 5)"
), levels = c(
  "ChildVaxView (Dose 1, Age 2)",
  "ChildVaxView (Dose 1, Age 3)",
  "SchoolVaxView (Dose 2, Age 5)",
  "TeenVaxView (Dose 2, Ages 14-18)",
  "6th Grade Survey (Dose 2, Age 11)",
  "Kindergarten Entry (Dose 2, Age 5)"
))]
...
# Compute analytical expected coverage mu for each stream
obs[loc_type == "State" & dose == 1 & age_min == 2, mu := (1 - phi_st[cohort]) * cov[2, 1]]
obs[loc_type == "State" & dose == 1 & age_min == 3, mu := (1 - phi_st[cohort]) * cov[3, 1]]
obs[loc_type == "State" & dose == 2 & age_min == 5, mu := (1 - phi_st[cohort]) * cov[5, 2]]
obs[loc_type == "State" & dose == 2 & age_min == 14, mu := sapply(cohort, function(c) {
  ages <- 14:18
  c_slices <- c + max(ages) - ages
  mean((1 - phi_st[c_slices]) * cov[ages, 2])
})]
obs[loc_type == "County", mu := {
  c_off <- unname(off_cnty[loc_id])
  phi_c <- plogis(qlogis(phi_st[cohort]) + c_off)
  (1 - phi_c) * cov[11, 2] * censor_red
}]
obs[loc_type == "School", mu := {
...
obs[, p_obs := positive / sample_n]
# Haldane-Anscombe continuity correction for empirical logit
obs[, p_adj := (positive + 0.5) / (sample_n + 1.0)]
obs[, delta_logit := qlogis(p_adj) - qlogis(mu)]

summary_table <- obs[order(loc_type, observation), .(
  `Observations` = .N,
  `Mean Observed` = sprintf("%.3f", mean(p_obs)),
  `Mean Expected` = sprintf("%.3f", mean(mu)),
  `Mean Log-Odds Residual` = sprintf("%+.4f", mean(delta_logit)),
  `SD Log-Odds Residual` = sprintf("%.4f", sd(delta_logit)),
  `Odds Ratio` = sprintf("%.4f", exp(mean(delta_logit)))
), by = .(
...
knitr::kable(
  summary_table,
  caption = paste(
    "Summary statistics of simulated observations compared to expected latent",
    "coverage by location type and observation stream."
  )
)
```

Additionally, breaking down the county-level and school-level observations by county layer
membership (Scruggs, Simone, Watson) confirms that the simulation maintains calibration across
individual geographic sub-hierarchies:

```{r observation-breakdown-table}
# Layer membership breakdown for County and School layers
obs[, county_group := fcase(
  loc_type == "State", "State",
  loc_type == "County", loc_id,
  default = parent_id
)]

breakdown_table <- obs[loc_type %in% c("County", "School"), .(
  `Observations` = .N,
  `Mean Observed` = sprintf("%.3f", mean(p_obs)),
  `Mean Expected` = sprintf("%.3f", mean(mu)),
  `Mean Log-Odds Residual` = sprintf("%+.4f", mean(delta_logit)),
  ...
  `Layer / Level` = loc_type,
  `County Membership` = county_group,
  `Observation Stream` = observation
)][order(`Layer / Level`, `County Membership`)]

knitr::kable(
  breakdown_table,
  caption = paste(
    "Breakdown of simulated observation log-odds residuals by geographic layer membership",
    "(Scruggs, Simone, Watson)."
  )
)
```

Finally, inspecting the individual school-level observations across all 24 schools illustrates
local sampling variability around each school's underlying latent trajectory:

```{r observation-school-table}
# Individual school-level summary statistics
school_table <- obs[loc_type == "School", .(
  `Observations` = .N,
  `Mean Observed` = sprintf("%.3f", mean(p_obs)),
  `Mean Expected` = sprintf("%.3f", mean(mu)),
  `Mean Log-Odds Residual` = sprintf("%+.4f", mean(delta_logit)),
...
knitr::kable(
  school_table,
  caption = paste(
    "Summary statistics of simulated kindergarten entry observations for all 24 individual",
    "schools compared to true latent coverage."
  )
)
```
...
We can also examine trace plots for key parameters to check MCMC convergence and evaluate parameter recovery against the true data-generating simulation parameters (`latent_params_sim`). In each trace plot, the distinct trace colors and corresponding horizontal solid lines represent individual MCMC sampling chains (chains 1–4) and their within-chain medians, while the dashed red lines and annotated values indicate the true simulation parameters.

#### Basis Spline Coefficients ($\beta_{\text{bs}}$)

Trace plots for the B-spline basis coefficients $\beta_{\text{bs}}$ (`beta_bs[1]` through `beta_bs[5]`) showing individual chain medians and true simulation parameters (dashed red lines and annotated values):
...
Trace plots for the hierarchy layer standard deviations $\sigma_{\text{county}}$ (`sigma_layer[1]`) and $\sigma_{\text{school}}$ (`sigma_layer[2]`) zoomed to the shared range $[0, 2.5]$ via coordinate clipping (preserving full chains), showing individual chain medians and true simulation standard deviations (dashed red lines and annotated values):
...
Trace plots for the county-level location offsets $\delta_{\text{county}}$ (`off_layer[1]` through `off_layer[3]`) showing individual chain medians and true simulation offsets (dashed red lines and annotated values):
...
Trace plots for school-level location offsets $\delta_{\text{school}}$ divided by county, showing individual chain medians and true simulation offsets (dashed red lines and annotated values) across schools in Scruggs, Simone, and Watson counties:

```{r trace-plot-school-offsets, fold = TRUE, code_summary = "Show plot code", results = "asis"}
...
Trace plots for the unconstrained dose uptake rates $\lambda_{\text{raw}}$ (`lambda_raw[1]` and `lambda_raw[2]`) zoomed to the shared range $[\exp(0.5), \exp(1.5)]$ via coordinate clipping (preserving full chains), showing individual chain medians and log-transformed true simulation parameters $\log(\lambda)$ (dashed red lines and annotated values) with an exponentiated y-axis scale and tick labels:
...
Finally, we can zoom into school-level coverage estimates across an entire county. Below, we compare the predicted coverage distributions across all schools within Scruggs County against the true underlying latent coverage from the simulation process:

```{r school-viz, fold = TRUE, code_summary = "Show plot code"}
