...
test_that("summary returns one row per target obs with target cols + stats", {
  ...
  expect_true(all(c("obs_c_id", "loc_id", "age", "cohort", "dose") %in% names(s)))
...
test_that("summary statistics match a manual reduction of the draws", {
  ...
  expect_equal(s$mean, colMeans(pred$draws), tolerance = 1e-12)
  manual_q <- t(apply(pred$draws, 2, stats::quantile, probs = c(0.025, 0.5, 0.975)))
...
test_that("custom probs control which quantile columns appear", {
  ...
  expect_equal(s$q10, unname(apply(pred$draws, 2, stats::quantile, probs = 0.1)),
               tolerance = 1e-12)
})

test_that("summary errors on a non-imugap_predict object", {
  expect_error(
    summary.imugap_predict(list(draws = matrix(0), target = data.table::data.table())),
