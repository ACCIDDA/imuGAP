...
parameters { // all of these are on the logit scale
  ...
  matrix[n_school, n_yr] s_dev; // school-level deviates
  real<lower=0> sd_dev;
}
...
transformed parameters {
  ...
  for(i in 1:n_cnty){
    matrix[cnty_end_index[i] - cnty_start_index[i] + 1, n_yr] dev_temp;
    for(j in 1:(cnty_end_index[i] - cnty_start_index[i] + 1)) {
      dev_temp[j,] = cumulative_sum(s_dev[cnty_start_index[i]+j-1,]);
    }
