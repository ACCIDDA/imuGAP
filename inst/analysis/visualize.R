...
res <- readRDS("../nc_measles/model_results/dashboard_outputs_10.7.25.rds") %>%
  select(-est3, -is_est)
...
st_est <- res %>%
  ...
  group_by(cohort) %>%
  summarize(
    est = sum(est_orig * population) / sum(population),
...
st_obs <- data_dt %>%
  ...
  group_by(cohort) %>%
  summarize(
    up_to_date = sum(all_positive, na.rm = T) /
      sum(population[!is.na(all_positive)]),
    non_exempt = sum(population - exempt_any_tot, na.rm = T) /
      sum(population[!is.na(exempt_any_tot)])
  ) %>%
  pivot_longer(-cohort)

ggplot() +
  geom_line(
    data = st_est %>% filter(cohort != 2021),
    aes(x = cohort, y = est)
  ) +
  geom_ribbon(
    data = st_est %>% filter(cohort != 2021),
    aes(x = cohort, ymin = ci_low, ymax = ci_high),
    alpha = 0.5,
    fill = "gray"
  ) +
...
res %>%
  ...
  mutate(est_vax = est_orig * population, act_vax = est * population) %>%
