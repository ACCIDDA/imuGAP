...
for (i in 1:length(models)) {
  ch[[i]] <- rstan::extract(readRDS(paste0(
    "../nc_measles/model_results/process_",
    models[i],
    "_pred.rds"
  )))
  pred_est[[i]] <- data.frame(
    est = apply(ch[[i]]$p_gen, 2, mean),
...
for (i in unique(sch_enr$unit_id)) {
  ...
  if (min(tmp$cohort) == 2010) {
    fill_enr[[ct]] <- data.frame(
      enc_unit_id = tmp$enc_unit_id[1],
      unit_id = i,
...
sch_enr <- bind_rows(sch_enr, fill_enr) %>%
  arrange(enc_unit_id, unit_id, cohort)

cohort_key$age_group <- paste(
  seq(from = 36, to = 4, by = -1),
...
# Join empirical data
emp <- res_dt %>%
  filter(!is.na(all_positive), !is.na(population), population != 0) %>%
  mutate(cohort = year - 5) %>%
  select(
    -vax_positive,
    -exempt_vax_med,
  ...
  mutate(
    pct_exempt = (exempt_any_rel + exempt_any_med) / population,
...
dash_out <- pred_res %>%
  ...
  filter(dose == 2) %>%
  rename(year = cohort_norm, cnty_id = enc_unit_id, id = unit_id) %>%
  mutate(est_orig = est) %>%
