...
transformed data {
  ...
  vector[n_cohort] cohort_shift_counter = linspaced_vector(n_cohort, 1,
                                                           n_cohort);
  ...
  for (weight_i in 1 : n_weights) {
    ...
    phi_lookup[weight_i] = weights_cohort[weight_i]
                           + (weights_location[weight_i] - 1) * n_cohort;
  ...
  for (k in 1 : (n_layers - 1)) {
    loc_layer_idx[layer_bounds[1, k + 1] - 1 : layer_bounds[2, k + 1] - 1] = rep_array(k,
                                                                    layer_sizes[k
                                                                    + 1]);
  }
...
model {
  if (!predict_mode) {
    ...
    for (p in 1 : n_parent_locs) {
      ...
      logit_phi_loc[st : en] = logit_phi_loc[parent_loc_id[p]]
                               + off_layer[(st - 1) : (en - 1)];
    }
    
    matrix[n_cohort, n_locs] logit_phi_mat = rep_matrix(logit_phi_st, n_locs)
                                             + rep_matrix(to_row_vector(
                                                          logit_phi_loc),
                                                          n_cohort);
