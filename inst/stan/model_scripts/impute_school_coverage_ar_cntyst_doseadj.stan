...
data {
  ...
  array[n_cnty] int cnty_start_index;
  array[n_cnty] int cnty_end_index;
  
  // weights
  matrix[n_school, n_yr] sch_wts; // school weights = enrollment at school/total enrollment in county
  array[3] matrix[n_cnty, n_yr] pop_wts; // county population weights = county pop for age group/state pop for age group for 19-35 mos (1), kindergarten entry (2), and 13-15 yos (3)
  
  // school-level observations
  array[n_sch_obs, 2] int y_sch; // observed school-level num up to date (col 1) and total enrollment (col 2)
  array[n_sch_obs] int y_sch_id; // school IDs
  array[n_sch_obs] int y_sch_yr; // years
  
  // state-level observations
  array[n_state_obs, 2] int y_state; // state-level coverage observations num up to date (col 1) and total enrollment (col 2)
  matrix[n_state_obs, n_yr] y_state_grades; // weights for grade cohorts corresponding to state-level observations
  array[n_state_obs] int y_state_pop; // 19-35 mos (1), kindergarten entry (2), and 13-15 yos (3)
}
...
parameters { // all of these are on the logit scale
  ...
  array[n_cnty] real<lower=0> sd_cnty_ar;
