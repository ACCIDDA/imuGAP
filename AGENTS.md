...
* **Tracked vs. Untracked Artifacts**:
  ...
  * Fitted data objects (`data/fit_sim*.rda`, `data/predict_sim*.rda`, `data/target_sim*.rda`, `data/leave_school_out*.rda`) are generated by `just data-fit` and are untracked in git.
...
* **Dependency Minimization & `usethis` Tooling**:
  * Prefer using base R or existing dependencies (`data.table`, `testthat`, etc.) rather than
    introducing new package dependencies. Avoid adding convenience packages (e.g. `rprojroot`,
    `fs`, `glue`) when existing tooling or standard base functions suffice.
  * When a new dependency is genuinely warranted, always add it to `DESCRIPTION` using
    `usethis::use_package("<pkg>", type = "Imports"|"Suggests")` to maintain standardized
    formatting, valid field classification, and alphabetical ordering.
